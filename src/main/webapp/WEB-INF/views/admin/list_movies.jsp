  ...
  <%@ include file="addMenuAside.jsp" %>
    <div class="page-holder w-100 d-flex flex-wrap">
      ...
      <div class="container-fluid px-xl-5">
        <section class="py-5">   
           <button class="btn btn-primary col-lg-3 ml-6" type="button" onclick=" window.location.href='<c:url value="/manager/showFormForMovie"/>'">Create Movie</button>
        </section>
...
<!-- bắt đầu bảng -->
                ...
                <div class="card">
                  <div class="card-header">
                    <h6 class="text-uppercase mb-0">List Movie</h6>
                  </div>
                  <div class="card-body" style="overflow-x:auto;">                           
                    <table class="table table-striped table-hover card-text " id="dtHorizontalExample">
                      ...
                      <c:forEach items="${suggests}" var="msug">	 
                      	<c:url var="deleteLink" value="deleteMovie">
							<c:param name="movieId" value="${msug.id_film}"/>
						</c:url>
						<c:url var="updateLink" value="editMovie">
							<c:param name="movieId" value="${msug.id_film}"/>
						</c:url>     		
						<tr >
			          		<td><img src="${images}/${msug.image}" alt="${images}/${msug.image}"></td>
			          		<td>${msug.quality}</td>
			          		<td>${msug.name_film}</td>
			          		<td>${msug.time} phút</td>
			          		<td>${msug.status}</td>
			          		<td>${msug.open_date}</td>
			          		<td><img src="${images}/item-img/${msug.image_detail1}" alt="${images}/${msug.image_detail1}"></td>
			          		<td><img src="${images}/item-img/${msug.image_detail2}" alt="${images}/${msug.image_detail2}"></td>
			          		<td class="description">${msug.description}</td> 
			          		<td>${msug.link_film}</td>
			          		<td>${msug.link_trailer}</td>
			          		<td>${msug.new_film}</td>
			          		<td>${msug.is_series}</td>
...
<!-- kết thúc bảng -->
  ...
  (function() {
	    document.getElementById("moviemanageractive").className += " active";
		})();
  </script>
  <!-- JavaScript files-->
