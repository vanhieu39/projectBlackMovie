  ...
  <div class="d-flex align-items-stretch">
    ...
    <div class="page-holder w-100 d-flex flex-wrap">
      ...
      <div class="container-fluid button-add">
           <button class="btn btn-primary col-lg-3 ml-6" type="button" onclick=" window.location.href='<c:url value="/manager/showFormAddDirector"/>'">Add Director</button>
...
<!-- bắt đầu bảng -->
                ...
                <div class="card">
                  <div class="card-header">
                    <h6 class="text-uppercase mb-0">List Directors</h6>
                  </div>
                  <div class="card-body">                           
                    <table class="table table-striped table-hover card-text">
                      ...
                      <tbody>
                        <c:forEach items="${ listdirector}" var="e">
							<c:url var="deleteLink" value="/manager/deleteDirector">
								<c:param name="id" value="${e.id}"/>
							</c:url>
							<c:url var="updateLink" value="/manager/updateDirector">
								<c:param name="id" value="${e.id}"/>
							</c:url>
							<tr>
								<td>${e.fullname }</td>
							    ...
							    <c:if test = "${e.gender != 0 && e.gender != 1}">
							      <td>Bi</td>
							    </c:if>
								...
								<td>${e.nation }</td>
								<td><a href="${e.images }">${e.images }</a></td>
								<td>${e.view }</td>
...
<!-- kết thúc bảng -->
  ...
  (function() {
	    document.getElementById("directorsnamageractive").className += " active";
		})();
  </script>
  <!-- JavaScript files-->
